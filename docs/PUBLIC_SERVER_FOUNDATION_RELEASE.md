...
2. PatchBukkit Java/protobuf and conformance-plugin JAR build, including
   ...
   interaction variants share one native subscription and player lifecycle
   events preserve the canonical Bukkit player wrapper.
...
6. Confirm the workflow logs contain no `hasMetadata`, Java
   global-reference, panic, disconnect, restart, `Player not found for UUID`,
   invalid `PlayerJoinEvent` response, `EssentialsTimer Error`, or
   `setSleepingIgnored` errors.
...
8. Let Essentials' AFK timer run and confirm the player's sleep-ignore state
   changes without an exception.
